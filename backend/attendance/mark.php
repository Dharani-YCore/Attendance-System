...
if (!empty($data->user_id) && !empty($data->status)) {
    ...
    if (isset($data->qr_data)) {
        ...
        $qrInfo = json_decode($data->qr_data, true);
        if (isset($qrInfo['user_latitude']) && isset($qrInfo['user_longitude'])) {
        ...
        logQRScan($data->user_id, $qrValidation['qr_info'], $latitude, $longitude, $db);
    }
    ...
    if ($existingAttendance) {
        ...
        if ($existingAttendance['check_out_time'] === null) {
            ...
            // Also populate CRM-compatible out_time column so CRM Attendance Reports can see it
            $query = "UPDATE $attendanceTable SET check_out_time = ?, out_time = ?, total_hours = ?, attendance_type = 'full_day', check_out_latitude = ?, check_out_longitude = ?, check_out_address = ?, location_accuracy = ? WHERE id = ?";
            $stmt = $db->prepare($query);
            $stmt->bindParam(1, $checkOutTime);
            $stmt->bindParam(2, $checkOutTime);
            $stmt->bindParam(3, $totalHours);
            $stmt->bindParam(4, $latitude);
            $stmt->bindParam(5, $longitude);
            $stmt->bindParam(6, $address);
            $stmt->bindParam(7, $locationAccuracy);
            $stmt->bindParam(8, $existingAttendance['id']);
        ...
        // Also populate CRM-compatible in_time column so CRM Attendance Reports can see it
        $query = "INSERT INTO $attendanceTable SET user_id=:user_id, date=:date, time=:time, in_time=:in_time, check_in_time=:check_in_time, status=:status, attendance_type='check_in', check_in_latitude=:latitude, check_in_longitude=:longitude, check_in_address=:address, location_accuracy=:accuracy";
        $stmt = $db->prepare($query);
