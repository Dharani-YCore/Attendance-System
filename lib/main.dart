  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed && 
      ...
      if (authProvider.isLoggedIn && _isAuthenticated) {
        _checkAndRequireBiometricAuth();
      }
    }
    _lastLifecycleState = state;
  }
  
  Future<void> _checkAndRequireBiometricAuth() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final biometricEnabled = prefs.getBool('biometric_enabled') ?? true; // Default to enabled
      ...
      if (biometricEnabled) {
        ...
        // If biometric is disabled, keep user authenticated
        print('🔓 Biometric authentication disabled in preferences');
      }
    } catch (e) {
      print('⚠️ Error checking biometric preference: $e');
      // Default to requiring biometric auth if there's an error
      setState(() {
        _isAuthenticated = false;
        _needsBiometricAuth = true;
      });
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        ...
        if (mounted) {
          setState(() {
            _initializing = false;
            // Set user as authenticated on normal app startup if logged in
            // Biometric auth will only be required when app resumes from background
