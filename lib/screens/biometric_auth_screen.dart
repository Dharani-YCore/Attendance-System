...
class _BiometricAuthScreenState extends State<BiometricAuthScreen> {
    ...
    try {
      ...
      if (mounted) {
        if (success) {
          ...
          setState(() {
            _errorMessage = 'Authentication failed. Please try again or use the skip option.';
  ...
  Future<void> _skipBiometricAuth() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('biometric_enabled', false);
      print('🔓 Biometric authentication disabled by user');
      
      if (mounted) {
        widget.onAuthenticated?.call();
      }
    } catch (e) {
      print('⚠️ Error disabling biometric authentication: $e');
      if (mounted) {
        setState(() {
          _errorMessage = 'Failed to disable biometric authentication. Please try again.';
        });
      }
    }
  }
